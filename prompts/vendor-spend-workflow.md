...
- Assign best-guess department based on name context.If vendor is unidentifiable assign G&A.
...
verbatim from the workbook), Spend, Category, and Department. Use those values as-is.
...
Do NOT re-read the workbook. Vendor names, spend, categories, departments, and suggestions are already in context from Steps 04 and 05. Use that data directly to assign strategic scores.
...
Validate every vendor row for completeness and accuracy:
...
Do NOT re-read the full Vendor Analysis Assessment sheet. All vendor classifications are already in context from Steps 04–08. Read ONLY the Top 3 Opportunities sheet from ${WORKBOOK} to verify savings figures.
...
Do NOT re-read the workbook. Vendor name, department, and suggestion are already in context from Steps 04–08. Use that data directly to assign confidence levels.
