...
Treat "Unknown" vendors as informational records only and exclude them from optimization opportunity analysis in step 09.
...
Output a rationale report as a structured report:
  Vendor Name | Recommendation | Rationale (1–2 sentences)

This log is for auditability and does not need to be written to the workbook. This output will be used to validate recommendation quality in Step 13.
...
- **Opportunity Title**: Descriptive title — max 6 words
- **Explanation**: 3 bullet points maximum, each bullet max 12 words. Format as:
  • [What the problem is — 1 bullet]
  • [What action to take — 1 bullet]
  • [Expected outcome / saving mechanism — 1 bullet]
  No full sentences. No paragraphs. Bullets only.
...
Write a brief first-person methodology explanation as if written by R Jegadeswari,
VP of Operations, who personally conducted this vendor spend assessment.

The A1 instruction asks for: approach, methodology, prompts used, and tools.
Cover all four in this order:

1. APPROACH — 2 sentences: how you structured the analysis end to end
2. METHODOLOGY — 3 sentences: how you classified vendors, identified opportunities,
   and developed recommendations. Reference the Terminate/Consolidate/Optimize framework.
3. PROMPTS — 2 sentences: describe the prompt-driven workflow used — 16 structured
   steps executed sequentially, each with a specific analytical task
4. TOOLS — 1 sentence: Claude Code CLI for AI reasoning, Node.js with xlsx library
   for Excel read/write, Python for formatting restoration

Tone: professional, first-person, concise.
Total length: maximum 8 sentences across all four sections.
Use plain prose — no bullet points, no numbered lists, no stage headers.

Comply with the template instructions in A1.
Do NOT overwrite the A1 instruction cell.
Write content to appropriate data cells only using XLSX.utils.encode_cell.
Do not use aoa_to_sheet or methods that rebuild the sheet.
<!-- /STEP:11 -->
...
Write a MAXIMUM 1-PAGE executive memo to the CEOCFO Recommendations sheet. Every sentence must earn its place. No preamble. No padding. No repetition.
...
- Year 1 conservatively realisable: $[amount] (60–70% of total)
